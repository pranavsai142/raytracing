# Handoff — Goethe chapter `vessel-elevation`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `vessel-elevation` case in `applyChapterPreset`).
...
| **Badge** | `PHYSICAL` |
| **Goethe saw** | Bottom of vessel appears raised through water. |
| **User must see** | Looking down; cube/bottom elevated by refraction; calm clear water. |
...
CHAPTER=vessel-elevation SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/13-vessel-elevation.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-vessel-elevation.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-vessel-elevation.md`
...
4. If FAIL visual: edit **only** `case 'vessel-elevation':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/vessel-elevation-RESULT.md`:
...
- Structural smoke PASS for `vessel-elevation`
