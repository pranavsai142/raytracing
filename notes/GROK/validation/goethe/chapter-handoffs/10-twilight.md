# Handoff — Goethe chapter `twilight`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `twilight` case in `applyChapterPreset`).
...
| **Badge** | `MIXED` |
| **Goethe saw** | Faint lights coloured at night; moon/candle twilight. |
| **User must see** | Low light; moon and/or warm candle presence; coloured faint lights; not blown-out day. |
...
CHAPTER=twilight SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/10-twilight.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-twilight.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-twilight.md`
...
4. If FAIL visual: edit **only** `case 'twilight':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/twilight-RESULT.md`:
...
- Structural smoke PASS for `twilight`
