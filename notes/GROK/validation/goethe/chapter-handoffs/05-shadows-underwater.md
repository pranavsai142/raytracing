# Handoff — Goethe chapter `shadows-underwater`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `shadows-underwater` case in `applyChapterPreset`).
...
| **Badge** | `MIXED` |
| **Goethe saw** | Divers: red-biased field, green-tending shadows underwater. |
| **User must see** | Underwater cube lit warm; cooler/greener shadows; cube in frame. |
...
CHAPTER=shadows-underwater SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/05-shadows-underwater.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-shadows-underwater.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-shadows-underwater.md`
...
4. If FAIL visual: edit **only** `case 'shadows-underwater':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/shadows-underwater-RESULT.md`:
...
- Structural smoke PASS for `shadows-underwater`
