# Handoff — Goethe chapter `ocean`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `ocean` case in `applyChapterPreset`).
...
| **Badge** | `PHYSICAL` |
| **Goethe saw** | North-star dielectric ocean: Fresnel, Snell, TIR trap/escape, spectral caustics. |
| **User must see** | Submerged checkerboard cube framed; surface with Snell/TIR; caustic light; no bulk blue paint. |
...
CHAPTER=ocean SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/01-ocean.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-ocean.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-ocean.md`
...
4. If FAIL visual: edit **only** `case 'ocean':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/ocean-RESULT.md`:
...
- Structural smoke PASS for `ocean`
