# Handoff — Goethe chapter `refraction`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `refraction` case in `applyChapterPreset`).
...
| **Badge** | `PHYSICAL` |
| **Goethe saw** | At refracting boundaries, displacement produces coloured fringes. |
| **User must see** | Cube + surface; chromatic edges/dispersion; water interface clear. |
...
CHAPTER=refraction SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/07-refraction.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-refraction.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-refraction.md`
...
4. If FAIL visual: edit **only** `case 'refraction':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/refraction-RESULT.md`:
...
- Structural smoke PASS for `refraction`
