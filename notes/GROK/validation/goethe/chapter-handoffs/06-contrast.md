# Handoff — Goethe chapter `contrast`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `contrast` case in `applyChapterPreset`).
...
| **Badge** | `MIXED` |
| **Goethe saw** | White against yellow takes purple physiological tint at boundary. |
| **User must see** | Yellow/white split floor visible; physiological layer on; boundary readable. |
...
CHAPTER=contrast SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/06-contrast.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-contrast.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-contrast.md`
...
4. If FAIL visual: edit **only** `case 'contrast':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/contrast-RESULT.md`:
...
- Structural smoke PASS for `contrast`
