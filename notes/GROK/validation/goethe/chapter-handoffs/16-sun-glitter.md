# Handoff — Goethe chapter `sun-glitter`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `sun-glitter` case in `applyChapterPreset`).
...
| **Badge** | `MIXED` |
| **Goethe saw** | Sun image on water surrounded by glittering halo. |
| **User must see** | Bright glitter path + bloom/halo on surface toward sun; readable. |
...
CHAPTER=sun-glitter SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/16-sun-glitter.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-sun-glitter.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-sun-glitter.md`
...
4. If FAIL visual: edit **only** `case 'sun-glitter':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/sun-glitter-RESULT.md`:
...
- Structural smoke PASS for `sun-glitter`
