# Handoff — Goethe chapter `wave-contrast`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `wave-contrast` case in `applyChapterPreset`).
...
| **Badge** | `MIXED` |
| **Goethe saw** | Agitated sea: lit faces vs opposite shadow contrast. |
| **User must see** | Visible wave slopes with lit/shadow contrast; not flat grey soup. |
...
CHAPTER=wave-contrast SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/14-wave-contrast.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-wave-contrast.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-wave-contrast.md`
...
4. If FAIL visual: edit **only** `case 'wave-contrast':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/wave-contrast-RESULT.md`:
...
- Structural smoke PASS for `wave-contrast`
