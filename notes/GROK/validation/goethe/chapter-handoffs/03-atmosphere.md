# Handoff — Goethe chapter `atmosphere`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `atmosphere` case in `applyChapterPreset`).
...
| **Badge** | `PHYSICAL` |
| **Goethe saw** | Sky blue = darkness through thin illumined vapour; sun through mist yellow→ruby. |
| **User must see** | Clear sky gradient (zenith blue vs warm solar haze); high renderScale; not muddy/blocky. |
...
CHAPTER=atmosphere SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/03-atmosphere.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-atmosphere.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-atmosphere.md`
...
4. If FAIL visual: edit **only** `case 'atmosphere':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/atmosphere-RESULT.md`:
...
- Structural smoke PASS for `atmosphere`
