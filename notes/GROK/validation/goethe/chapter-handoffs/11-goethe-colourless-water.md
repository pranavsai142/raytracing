# Handoff — Goethe chapter `goethe-colourless-water`

**You own only this chapter.** Parallel agents own the others — do not edit other chapters' presets casually (touch only the `goethe-colourless-water` case in `applyChapterPreset`).
...
| **Badge** | `PHYSICAL` |
| **Goethe saw** | Water has no colour; semi-opacity not pigment. |
| **User must see** | Cube through colourless medium; white/neutral scatter; no bulk blue dye. |
...
CHAPTER=goethe-colourless-water SAMPLES=64 RENDER_SCALE=1 SPP=2 npm run smoke:goethe:one
```

Outputs:
- PNG: `notes/GROK/validation/goethe/11-goethe-colourless-water.png`
- Verdict stub: `notes/GROK/validation/goethe/VERDICT-goethe-colourless-water.md`
- Report: `notes/GROK/validation/goethe/SMOKE_REPORT-goethe-colourless-water.md`
...
4. If FAIL visual: edit **only** `case 'goethe-colourless-water':` inside `src/PathTracer.ts` → `applyChapterPreset`.
...
7. Write final report to `notes/GROK/validation/goethe/chapter-handoffs/goethe-colourless-water-RESULT.md`:
...
- Structural smoke PASS for `goethe-colourless-water`
